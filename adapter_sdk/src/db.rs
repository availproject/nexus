...
impl<P: RollupProof + Clone + DeserializeOwned + Serialize, ZP: ZKVMProof + DeserializeOwned + Serialize + Clone> DB<P, ZP> {
    ...
    pub(crate) fn store_last_known_queue(&self, queue: &VecDeque<QueueItem<P>>) -> Result<(), Error> {
    ...
    pub(crate) fn get_last_proof(&self) -> Result<Option<(ZP, AdapterPublicInputs, u32)>, Error> {
        Ok(self.0.get::<(ZP, AdapterPublicInputs, u32)>(b"last_proof")?)
    }

    pub(crate) fn store_last_proof(&self, proof: &(ZP, AdapterPublicInputs, u32)) -> Result<(), Error> {
