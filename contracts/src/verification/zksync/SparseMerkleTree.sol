...
import {Blake2S} from "./Blask2s.sol";
...
contract SparseMerkleTree {
    ...
    function getRootHash(bytes32[] calldata proof, TreeEntry memory entry, address account)
        public
        view
        returns (bytes32)
    {
    ...
    function hashBranch(Blake2S.BLAKE2S_ctx memory ctx, bytes32 left, bytes32 right)
        internal
        view
        returns (bytes32 result)
    {
        uint256[2] memory DEFAULT_EMPTY_INPUT;
        Blake2S.init(ctx, 32, "", DEFAULT_EMPTY_INPUT, DEFAULT_EMPTY_INPUT);
    ...
    function hashLeaf(Blake2S.BLAKE2S_ctx memory ctx, uint64 leafIndex, bytes32 value)
        internal
        view
        returns (bytes32)
    {
