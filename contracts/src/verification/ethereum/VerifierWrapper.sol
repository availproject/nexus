...
contract VerifierWrapper is INexusVerifierWrapper, EthereumVerifier {
    ...
    constructor(bytes32 _nexusAppID, INexusProofManager _nexus) EthereumVerifier(_nexus) {
        nexusAppID = _nexusAppID;
        nexus = _nexus;
    }

    function parseAndVerify(uint256 chainblockNumber, bytes32 receipt, bytes calldata data, address from) external {
