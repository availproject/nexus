...
library JellyfishMerkleTreeVerifier {
    ...
    function verifyProof(bytes32 root, Leaf memory leaf, Proof memory proof) internal pure returns (bool) {
        ...
        uint8[] memory bitValue = calculateBits(leaf.addr, proof.siblings.length);
    ...
    function calculateBits(bytes32 elementKey, uint256 siblingsLen) public pure returns (uint8[] memory) {
        uint256 skipBits = 256 - siblingsLen;
        ...
        for (uint256 i = 255; i >= skipBits && index < siblingsLen; i--) {
            // Extract the bit at position i
            uint256 bit = (uint256(elementKey) >> i) & 1;
            result[index] = uint8(bit);
            index++;
        }

        // Reverse the array to match the expected order
        for (uint256 i = 0; i < siblingsLen / 2; i++) {
            uint8 temp = result[i];
