...
async fn run(loop_delay_mins: u64) -> Result<(), anyhow::Error> {
    ...
    let app_id = AppId(
        std::env::var("APP_ID")
            ...
            .parse()
            .expect("APP_ID should be a valid number"),
    );
    let avail_url = std::env::var("AVAIL_URL").unwrap_or_else(|_| String::from("wss://turing-rpc.avail.so:443/ws"));
    ...
    let nexus_api_url = std::env::var("NEXUS_API_URL").unwrap_or_else(|_| "http://127.0.0.1:7000".to_string());
    debug!("Initializing nexus on url: {}", nexus_api_url);
    ...
    loop {
        ...
        let start_nexus_hash = match init_account_if_not_initialized(
        ...
        match request_update_and_create_proof(
            client,
            last_head,
...
async fn init_account_if_not_initialized(
...
async fn request_update_and_create_proof<'a>(
