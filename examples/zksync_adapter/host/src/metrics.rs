...
impl AdapterMetrics {
    pub fn init() -> Self {
        ...
        let proving_block_time_histogram = metrics_meter.u64_histogram("proving_block_time").with_unit("secs").init();
        let blocks_proved = metrics_meter
            .u64_counter("zksync_block_proved")
            .with_description("Number of zksync blocks proved")
            .init();
        Self {
            block_proving_time: proving_block_time_histogram,
            latest_height: proving_block_latest_height_gauge,
            blocks_proved,
        }
    }
}
